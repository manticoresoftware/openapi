...
(function(root, factory) {
  if (typeof define === 'function' && define.amd) {
    ...
    root.ManticoreSearchApi.SearchApi = factory(root.ManticoreSearchApi.ApiClient, root.ManticoreSearchApi.ErrorResponse, root.ManticoreSearchApi.PercolateRequest, root.ManticoreSearchApi.SearchRequest, root.ManticoreSearchApi.SearchResponse);
  }
  ...
  var exports = function(apiClient) {
    ...
    /**
     * Performs a search
     * Performs a search.  Expects an object with mandatory properties: - the index name <br/> - the match query object <br/> Example :    ```   {'index':'movies','query':{'bool':{'must':[{'query_string':' movie'}]}},'script_fields':{'myexpr':{'script':{'inline':'IF(rating>8,1,0)'}}},'sort':[{'myexpr':'desc'},{'_score':'desc'}],'profile':true}   ```  It responds with an object with: - time of execution - if the query timed out - an array with hits (matched documents) - additional, if profiling is enabled, an array with profiling information is attached     ```   {'took':10,'timed_out':false,'hits':{'total':2,'hits':[{'_id':'1','_score':1,'_source':{'gid':11}},{'_id':'2','_score':1,'_source':{'gid':12}}]}}   ```  For more information about the match query syntax, additional paramaters that can be set to the input and response, please check: https://docs.manticoresearch.com/latest/html/http_reference/json_search.html. 
