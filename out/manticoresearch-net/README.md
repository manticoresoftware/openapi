...
To generate a netstandard2.1 code you can run generator from docker:

```powershell
docker run --rm -v <local_folder>:/local openapitools/openapi-generator-cli generate -i /local/manticore.yml -g csharp-netcore --library httpclient --additional-properties=targetFramework=netstandard2.1 --additional-properties=packageName=ManticoreSearch -o /local/out/csharp
```
You can get the manticore.yml from [here](https://github.com/manticoresoftware/openapi/blob/master/manticore.yml).
